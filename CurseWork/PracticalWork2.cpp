...
#define N2 100 //позволяет как бы переменно задавать размер массивов, так как компилятор не дает использовать переменные, кроме констант
...
void userFindLess(int* arr) {
	...
	if (ab < arr[0])
		...
		if (ab > arr[N2 - 1])
			...
			double countD = findPlace(arr, 0, N2 - 1, ab);
...
void userFindMore(int* arr) {
	...
	if (ab > arr[N2 - 1])
		...
		if (ab < arr[0])
			...
			while (arr[count] == ab)
				count++;
			std::cout << N2 - count << " элементов больше чем " << ab << "\n\n";
		}
	}
}
...
void userFindNumber(int* arr) {
	...
	start = std::chrono::system_clock::now();
	countBin = findElem(arr, 0, N2 - 1, a);
	end = std::chrono::system_clock::now();
	binFindTime = end - start;
	start = std::chrono::system_clock::now();
	countSimp = simpFindElem(arr, 0, N2 - 1, a);
	end = std::chrono::system_clock::now();
